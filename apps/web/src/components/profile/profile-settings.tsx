...
function SettingRow({ label, description, checked, icon: Icon, onChange }: SettingRowProps) {
  return <Card className="flex items-center gap-4 rounded-2xl p-4"><Icon aria-hidden="true" size={20} className="shrink-0 text-(--ink-muted)" /><div className="min-w-0 flex-1"><h3 className="font-semibold">{label}</h3><p className="mt-1 text-sm leading-5 text-(--ink-muted)">{description}</p></div><button type="button" role="switch" aria-checked={checked} aria-label={label} onClick={onChange} className={`relative h-7 w-12 shrink-0 rounded-full transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--accent) ${checked ? "bg-(--action-bg)" : "bg-(--surface-muted)"}`}><span className={`absolute left-1 top-1 size-5 rounded-full bg-(--surface) transition-transform ${checked ? "translate-x-5" : "translate-x-0"}`} /></button></Card>;
}
